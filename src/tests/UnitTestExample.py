...
if __name__ == "__main__":
    ...
    for a in range(len(input_floats)):
        pred_value = computePrediction(torch.Tensor(
            [input_floats[a]]), action_ints[a], example_model).item()
    ...
    for a in range(len(input_floats)):
        target_value = computeTargets(reward_floats[a], torch.Tensor(
            [input_floats[a]]), discountFactor, dones_bool[a], example_model).item()
